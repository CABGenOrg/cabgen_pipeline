...
montagem = (f"{path.basename(caminho1)}/{path.basename(sample)}"
...
if resultado_final_especie == 'pseudomonasaeruginosa':
    ...
    fasta_polimixina = (f"{path.basename(db_polimixina)}/"
...
# MELISE: ADICIONEI
    ...
    if resultado_final_especie == 'klebsiellapneumoniae':
        ...
        fasta_outros = (f"{path.basename(db_outrosMut)}"
...
MLST_result = (f"{path.basename(caminho1)}/{path.basename(sample)}"
               "/unicycler/data.json")
